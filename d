...
class Space:
    ...
    bullets_count=0,score=0,score_position=(30,10),lives=0, lives_position=(550,10),stage=1,left_border=False,
    right_border=False,right_movement=True,game_clock=pygame.time.Clock(),game_over=True):
...
def load_game_over_files(path):
    ...
    while loop <= 2:
        ...
        if loop == 0:
            ...
            image_1 = pygame.image.load(image_name).convert_alpha()

        elif loop == 1:

            image_name = path + r'\retry_1.png'
            image_2 = pygame.image.load(image_name).convert_alpha()

            image_name = path + r'\retry_2.png'
            image_3 = pygame.image.load(image_name).convert_alpha()

        else:

            image_name = path + r'\title_1.png'
            image_4 = pygame.image.load(image_name).convert_alpha()

            image_name = path + r'\title_2.png'
            image_5 = pygame.image.load(image_name).convert_alpha()
...
#file_name = ".\images\game_over\game_over.png"
#game_over = pygame.image.load(file_name).convert_alpha()
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if Game_Space.game_over is True:
            ...
            if event.type == KEYDOWN:
                ...
                if event.key == K_DOWN and retry_button_active is True:

                    retry_button_active = False
                    title_button_active = True

                if event.key == K_UP and title_button_active is True:
    ...
    if Game_Space.game_over is False:
        ...
        if game_over is retry_active is retry_inactive is title_active is title_inactive is None:

            image_path = ".\images\game_over"
            game_over, retry_active, retry_inactive, title_active, title_inactive = load_game_over_files(image_path)

        else:

            screen.blit(pygame.transform.scale(game_over, (350, 350)), (125, -30))

            if retry_button_active is True:

                screen.blit(pygame.transform.scale(retry_active, (225, 100)), (185, 275))

            else:

                screen.blit(pygame.transform.scale(retry_inactive, (225, 100)), (185, 275))

            if title_button_active is True:

                screen.blit(pygame.transform.scale(title_active, (550, 100)), (50, 425))

            else:

                screen.blit(pygame.transform.scale(title_inactive, (550, 100)), (50, 425))
